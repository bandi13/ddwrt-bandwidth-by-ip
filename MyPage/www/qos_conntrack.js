...
function setPORTTable() {
	...
	for(var i=0;i<clients.length;i++){
		...
		for(var p=0;p<ports.length;p++){
			var cons = 0;
			for (var c in ports[p]) { if (ports[p][c] == clients[i]) cons++; }
...
addEvent(window, "load", function() {
	...
	update.onUpdate("arp", function(u) { eval('setArp(' + u.arp + ')'); }); 
	update.onUpdate("ip_conntrack", function(u) { eval('setCONTable(' + u.ip_conntrack + ')'); });
	update.onUpdate("bw_table", function(u) { eval('setBWTable(' + u.bw_table + ')'); });
	update.start();
});
addEvent(window, "unload", function() { update.stop(); });
//]]>
